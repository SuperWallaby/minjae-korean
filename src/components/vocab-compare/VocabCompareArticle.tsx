...
export function VocabCompareArticle({ page }: Props) {
  const playHref = vocabQuizPlayPath(`vocab-compare-${page.slug}`);
    ...
    <article className="space-y-10">
      <header className="space-y-3">
        ...
        <p className="text-base text-[var(--quiz-text-sub)]">
          {page.left.korean}
          <span className="mx-2 text-[var(--quiz-text-muted)]">vs</span>
          {page.right.korean}
        </p>
      </header>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <WordPanel
          ...
          whenToUseHref={page.left.whenToUsePath}
        />
        <WordPanel
          ...
          whenToUseHref={page.right.whenToUsePath}
        />
      </div>

      <section className="space-y-3" aria-labelledby="vocab-compare-diff">
        <h2
          id="vocab-compare-diff"
      ...
      <aside className="rounded-[1.25rem] border border-[var(--quiz-border)] bg-[var(--quiz-surface-muted)] px-5 py-5">
        ...
        <p className="mt-1 text-sm text-[var(--quiz-text-sub)]">
          Picture quizzes use the same illustrations — listen, pick the answer, keep going.
        </p>
        <Link
          href={playHref}
...
export function VocabCompareHubCard(props: VocabCompareListItem) {
  return (
    <Link
      href={vocabComparePath(props.leftId, props.rightId, props.slug)}
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-sm text-[var(--quiz-text-sub)]">
          {props.left.korean}
          <span className="text-[var(--quiz-text-muted)]"> vs </span>
          {props.right.korean}
        </p>
      </div>
    </Link>
