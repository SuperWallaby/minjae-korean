...
export function WhenToUseArticle({ page }: Props) {
  const playHref = vocabQuizPlayPath(`when-to-use-${page.slug}`);
  ...
  return (
    <article className="space-y-10">
      ...
      <section className="space-y-3" aria-labelledby="when-to-use-explanation">
        <h2
          id="when-to-use-explanation"
      ...
      <section className="space-y-4" aria-labelledby="when-to-use-examples">
        <h2
          id="when-to-use-examples"
          className="text-lg font-semibold text-[var(--quiz-text)]"
        >
          Example sentences
        </h2>
...
export function WhenToUseHubCard(props: HubCardProps) {
  return (
    <Link
      href={whenToUsePath(props.id, props.slug)}
