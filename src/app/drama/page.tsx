      ...
      <Container className="max-w-5xl">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-3 text-sm text-muted-foreground sm:text-base">
              Learn Korean through drama. Click on lines to see translations and
              explanations.
            </p>
          </div>
          {isDev ? (
        ...
        {items.length === 0 ? (
          ...
          <>
            {major ? (
              <Link
                href={`/drama/${encodeURIComponent(major.slug)}`}
            ...
            {rest.length > 0 ? (
              <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {rest.map((d) => (
                  <Link
                    key={d.slug}
                    href={`/drama/${encodeURIComponent(d.slug)}`}
                  ...
                  >
                    <div className="relative aspect-video w-full overflow-hidden bg-muted/20">
                      {d.images?.thumb?.trim() || d.images?.large?.trim() ? (
                        <Image
                          src={
                            d.images?.thumb?.trim() ||
                            d.images?.large?.trim() ||
